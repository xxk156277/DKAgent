...
const STORED_EVENT_FIELDS = new Set([
    "schemaVersion",
    "eventId",
    ...EVENT_FIELDS,
    "worktreePath",
...
const SENSITIVE_FLAG =
    /(?:^|\s)--[a-z0-9-]*(?:api-key|apikey|token|secret|password|passphrase|authorization|auth|credential|private-key)[a-z0-9-]*(?:\s+\S+|=\S+|$)/i;
...
const ACK_INTENT_FIELDS = new Set([
    "schemaVersion",
    "lockOwner",
    "eventIds",
...
function validDocumentHashes(hashes) {
    return (
        ...
        DOCUMENTS.every(
            (relative) =>
                Object.hasOwn(hashes, relative) &&
                (hashes[relative] === null ||
                    (typeof hashes[relative] === "string" && DOCUMENT_HASH.test(hashes[relative]))),
        )
    );
}
...
function sameDocumentHashes(left, right) {
    return (
        validDocumentHashes(left) &&
        validDocumentHashes(right) &&
...
function validAckIntent(intent) {
    return (
        ...
        intent.eventIds.every(
            (eventId, index) => index === 0 || intent.eventIds[index - 1].localeCompare(eventId) < 0,
        ) &&
...
function ackIntentMatchesRequest(intent, owner, eventIds, expectedDocumentHashes) {
    return (
        sameLockOwnerIdentity(intent.lockOwner, owner) &&
...
function validLifecycleIntent(intent) {
    ...
    return (
        new Set(["ack", "adopt", "release", "recover"]).has(intent.operation) &&
...
function lockDiagnostics(root) {
    ...
    const blockingTombstones = recoveryTombstones.filter((name) => readLockOwner(path.join(root, name))).sort();
    return {
        recoveryTombstones,
        blockingTombstones,
...
function sameAckLifecycleRequest(intent, request) {
    return (
        intent.lockToken === request.lockToken &&
...
function isAllowedValidationCommand(tokens) {
    ...
    if (executable === "npm") {
        return (
            (tokens.length === 2 && subcommand === "test") ||
            (tokens.length === 3 && subcommand === "run" && /^[a-zA-Z0-9:_-]+$/.test(tokens[2] || ""))
        );
    }
    if (executable === "node") {
        return (
            (subcommand === "--test" && areTestPaths(tokens.slice(2))) ||
            (subcommand === "--check" &&
                tokens.length === 3 &&
                PATH_TOKEN.test(tokens[2]) &&
                NODE_CHECK_PATH.test(tokens[2]))
        );
    }
    ...
    if (executable === "python3")
        return (
            tokens.length === 3 &&
            PATH_TOKEN.test(subcommand || "") &&
            /quick_validate\.py$/.test(subcommand) &&
            PATH_TOKEN.test(tokens[2])
        );
    if (executable === "tsx") return subcommand === "--test" && areTestPaths(tokens.slice(2));
    if (executable === "npx") {
        return (
            (subcommand === "tsx" && tokens[2] === "--test" && areTestPaths(tokens.slice(3))) ||
...
export function stateRoot(cwd) {
    ...
    const commonDirectory = canonicalExistingPath(
...
function validateInput(input) {
    ...
    for (const evidence of input.evidence) {
        ...
        if (
            !EXECUTION_EVIDENCE.has(evidence.kind) &&
            (evidence.command !== undefined || evidence.exitCode !== undefined)
        ) {
...
function readProcessedEvents(root) {
    ...
    return events.sort(
        (left, right) => left.createdAt.localeCompare(right.createdAt) || left.eventId.localeCompare(right.eventId),
    );
}
...
function validPersistedState(state) {
    return (
        ...
        Object.entries(state.activeClaims).every(
            ([taskId, worktrees]) =>
                TASK_ID.test(taskId) &&
...
function auditProcessedDirectory(root) {
    ...
    const reasons = [];
    const events = [];
    for (const entry of readdirSync(processed, { withFileTypes: true }).sort((left, right) =>
...
function ackIntentRecoveryReasons(root, config, persistedState, intent, owner) {
    ...
    if (!owner || !sameDocumentHashes(intent.baselineHashes, owner.baselineHashes))
        reasons.push("ACK intent baseline differs from lock owner");
    ...
    const committedState =
        sameDocumentHashes(persistedState.documentHashes, intent.expectedDocumentHashes) &&
...
function completeAckCleanup(
    root,
    config,
    persistedState,
    intent,
    { allowedLiveMarker, _testTerminateAfterRenameCount, _testTerminationSignal } = {},
) {
...
function currentProjectionBlocks(managementWorktree) {
    ...
    for (const relative of ["docs/project/STATUS.md", "docs/project/BACKLOG.md"]) {
        ...
        for (let index = 0; index < lines.length - 1; index += 1) {
            if (!lines[index].startsWith(PROJECT_TASK_PREFIX) || !lines[index + 1].startsWith(PROJECT_EVENT_PREFIX))
                continue;
            let human;
            try {
...
export function acquireLock({ cwd }) {
    ...
    if (
        afterPublish.blockingTombstones.length ||
        afterPublish.intentMarkers.length ||
        currentOwner?.token !== owner.token ||
        currentInode !== publishedInode
    ) {
...
export function recoverAckCleanup({ cwd, confirm = false, _testTerminateAfterRenameCount, _testTerminationSignal }) {
...
export function ackEvents({
    ...
    eventIds = [...eventIds].sort();
    expectedDocumentHashes = Object.fromEntries(
        DOCUMENTS.map((relative) => [relative, expectedDocumentHashes[relative]]),
    );
    ...
    try {
        ...
        if (existingIntent.exists) {
            ...
            const recoveryReasons = ackIntentRecoveryReasons(
                root,
                config,
                persistedState,
        ...
        for (const event of latestEventsByTask(requestedEvents)) {
            ...
            if (taskBlocks.length !== 1) {
                throw new Error(
                    `event is not canonically projected; selected task must have exactly one current projection: ${event.taskId}`,
                );
            }
            const expected = projectionBlock(event);
            if (
...
async function main(args) {
    ...
    if (command === "snapshot")
        return readSnapshot({ cwd, token: args.includes("--token") ? valueAfter(args, "--token") : undefined });
