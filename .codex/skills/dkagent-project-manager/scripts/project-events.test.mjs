...
function documentHashes(cwd) {
    return Object.fromEntries(
        ["AGENTS.md", "docs/project/STATUS.md", "docs/project/BACKLOG.md"].map((relative) => {
...
function writeAckLifecycleIntent(
    cwd,
    name,
    { pid, lockToken, eventIds, expectedDocumentHashes, operation = "ack", includeLockToken = true },
) {
    writeFileSync(
        path.join(stateRoot(cwd), name),
        `${JSON.stringify({
...
test("emit rejects extra fields", () => {
    ...
    assert.throws(
        () =>
            emitEvent({
                cwd,
                input: started({
                    discoveredTodos: [{ summary: "Follow up", module: "agent", reason: "Gap", rawLog: "secret" }],
                }),
            }),
    ...
    assert.throws(
        () =>
            emitEvent({
                cwd,
                input: started({ evidence: [{ kind: "test", summary: "pass", exitCode: 0, rawLog: "secret" }] }),
            }),
...
test("emit requires canonical uppercase task IDs without surrounding whitespace", () => {
    ...
    for (const taskId of [" DKA-ABC", "DKA-ABC ", "dka-ABC", "DKA-abc", "DKA-AB", "DKA-AB_C"]) {
        assert.throws(() => emitEvent({ cwd, input: started({ taskId }) }), /taskId must use canonical format/);
    }
    assert.throws(
    ...
    assert.equal(
        emitEvent({ cwd, input: started({ taskId: "DKA-ABC", dependencies: ["DKA-DEPENDENCY"] }) }).taskId,
        "DKA-ABC",
    );
});
...
test("emit downgrades unproved or failed completed events without losing finished action", () => {
    ...
    assert.deepEqual(
        events.map(({ action, status }) => ({ action, status })),
        [
    ...
    assert.deepEqual(
        readSnapshot({ cwd }).events.map((event) => event.status),
    ...
    assert.throws(
        () =>
            emitEvent({
                ...
                input: started({
                    evidence: [
                        { kind: "test", summary: "pass", command: "TOKEN=example-secret npm test", exitCode: 0 },
                    ],
                }),
            }),
        /unsafe text content/,
    ...
    assert.throws(
        () =>
            emitEvent({
                cwd,
                input: started({
                    evidence: [{ kind: "test", summary: "pass", command: "npm test\nnpm run typecheck", exitCode: 0 }],
                }),
            }),
        ...
        () =>
            emitEvent({
                ...
                input: started({
                    action: "finished",
                    status: "completed",
                    evidence: [{ kind: "test", summary: "pass", exitCode: 0 }],
                }),
            }),
    ...
    for (const command of [
        ...
        assert.throws(
            () =>
                emitEvent({
                    cwd,
                    input: started({ evidence: [{ kind: "test", summary: "pass", command, exitCode: 0 }] }),
                }),
            /unsafe text content/,
        );
    }
            ...
            evidence: [
                {
                    kind: "test",
                    summary: "pass",
                    command: "node --test .codex/skills/dkagent-project-manager/scripts/project-events.test.mjs",
                    exitCode: 0,
                },
            ],
        }),
    ...
    assert.throws(
        () =>
            emitEvent({
                cwd,
                input: started({
                    evidence: [{ kind: "test", summary: "x-api-key: example-value", command: "npm test", exitCode: 0 }],
                }),
            }),
        /unsafe text content/,
    );
    assert.throws(
        () =>
            emitEvent({
                ...
                input: started({
                    discoveredTodos: [
                        { summary: "Follow up", module: "agent", reason: "Authorization: Bearer example-value" },
                    ],
                }),
            }),
        /unsafe text content/,
        ...
        () =>
            adoptDocuments({
                ...
                _testBeforeStateWrite() {
                    writeFileSync(path.join(root, "aggregate.lock"), `${JSON.stringify(replacement, null, 2)}\n`);
                },
            }),
...
test("projection helper rejects stored events with incomplete source facts", () => {
    ...
    for (const field of ["schemaVersion", "worktreePath", "branch", "headSha", "createdAt"]) {
        ...
        assert.throws(() => renderEventProjections({ cwd, token, eventIds: [event.eventId] }), /invalid stored event/);
    }
    releaseLock({ cwd, token });
});
...
test("snapshot reconciles orphan processed events without writing state", () => {
    ...
    assert.deepEqual(
        snapshot.recoveryEvents.map((event) => event.eventId),
        [eventId],
    );
...
test("a SIGKILL after ACK event moves permits exact-token takeover and state recovery", () => {
    ...
    assert.equal(snapshot.target.recoveryMode, "ack_intent");
    assert.deepEqual(
        snapshot.recoveryEvents.map((item) => item.eventId),
        [event.eventId],
    );
...
test("terminal ACK cleanup rejects document drift and preserves its journal", () => {
    ...
    assert.throws(() => recoverAckCleanup({ cwd }), /requires explicit confirmation/);
    ...
    assert.throws(() => recoverAckCleanup({ cwd, confirm: true }), /management document hashes differ from ACK intent/);
    assert.equal(existsSync(path.join(root, "ack-intent.json")), true);
...
test("terminal ACK cleanup rejects state missing an intent event ID", () => {
    ...
    assert.throws(() => recoverAckCleanup({ cwd, confirm: true }), /persisted state is missing ACK event IDs/);
    assert.equal(existsSync(path.join(root, "ack-intent.json")), true);
});
...
test("terminal ACK cleanup diagnoses an invalid processed event without consuming its journal", () => {
    ...
    assert.throws(() => recoverAckCleanup({ cwd, confirm: true }), /processed ACK event is invalid/);
    assert.equal(existsSync(path.join(root, "ack-intent.json")), true);
});
    ...
    const variants = [
        {
            ...
            mutate({ event, root }) {
                ...
                writeFileSync(
                    path.join(root, "events", "processed", `${extraId}.json`),
...
test("terminal ACK cleanup rejects stored completed events without canonical completion evidence", () => {
    const variants = [
        ...
        {
            name: "git diff disguised as test",
            evidence: [{ kind: "test", summary: "diff clean", command: "git diff --check", exitCode: 0 }],
        },
...
test("terminal ACK cleanup rejects a live related marker", () => {
    ...
    assert.equal(readSnapshot({ cwd }).target.recoveryMode, undefined);
    assert.throws(() => recoverAckCleanup({ cwd, confirm: true }), /live lifecycle marker blocks ACK cleanup/);
    assert.equal(existsSync(path.join(root, "ack-intent.json")), true);
...
test("terminal ACK cleanup rejects malformed and different-transaction markers", () => {
    for (const variant of ["malformed", "different"]) {
        ...
        assert.throws(() => recoverAckCleanup({ cwd, confirm: true }), /unrelated or invalid lifecycle marker/);
        assert.equal(existsSync(path.join(root, "ack-intent.json")), true);
        assert.equal(existsSync(markerFile), true);
        assert.equal(existsSync(path.join(root, "events", "processed", `${event.eventId}.json`)), true);
    }
...
test("ACK intent hard-blocks document, owner, token, event ID, and hash mismatches", () => {
    ...
    writeFileSync(
        lockFile,
        `${JSON.stringify({ ...owner, token: "00000000-0000-4000-8000-000000000001" }, null, 2)}\n`,
    );
    assert.equal(readSnapshot({ cwd }).target.safe, false);
...
test("finished ACK replaces the started current projection while processed events retain both facts", () => {
    ...
    assert.deepEqual(
        readSnapshot({ cwd }).state.processedEventIds.sort(),
        [startedEvent.eventId, finishedEvent.eventId].sort(),
    );
});
...
test("ack rejects mismatched human facts and incomplete canonical payloads", () => {
    ...
    writeFileSync(path.join(cwd, "docs", "project", "BACKLOG.md"), "# Backlog\n");
    const human = JSON.parse(valid.taskLine.slice(PROJECT_TASK_PREFIX_FOR_TEST.length));
    for (const patch of [{ status: "needs_verification" }, { module: "trace" }, { summary: "Stale summary" }]) {
...
test("three worktrees render the management documents before ACKing exact hashes", () => {
    ...
    for (const event of snapshot.events) {
        assert.deepEqual(
            retainedRecords.find((payload) => payload.eventId === event.eventId),
            event,
        );
    }
});
...
test("activated project-management documents and Skill describe the hash-checked ACK workflow", () => {
    ...
    const contracts = readFileSync(
        path.join(repositoryRoot, ".codex/skills/dkagent-project-manager/references/document-contracts.md"),
        "utf8",
    );
