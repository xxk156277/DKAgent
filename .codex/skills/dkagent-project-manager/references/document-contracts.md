...
For an ownerless terminal journal, snapshot and lock-status return `recoveryMode: "ack_cleanup"` only when branch and document hashes equal the intent, no intent ID remains pending, current projections are exact, and all related ACK markers match the journal and are dead. Cleanup performs a strict full-directory audit: every processed entry must be a canonical lowercase UUID `.json` regular file, parse as JSON, and contain the matching valid stored event. From all audited events sorted by `(createdAt,eventId)`, it derives canonical `processedEventIds` and `activeClaims`; the persisted state schema, hashes, ID array including order, and claims object must match those derived facts exactly. Extra, missing, reordered, malformed, or mismatched history is a hard stop. This mode runs only `recover-ack-cleanup --confirm`; ordinary ACK, lock recovery, adoption, release, and acquisition are forbidden.
