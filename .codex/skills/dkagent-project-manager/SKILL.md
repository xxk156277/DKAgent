...
- Use `recover-lock --confirm` only for an invalid/legacy ownerless lock or stale creating file when no lifecycle marker exists. Any lifecycle marker is a hard stop regardless of recorded PID or apparent age; `recover-lock` never deletes a competing marker. A valid owner or any owned recovery tombstone is also a hard stop.
