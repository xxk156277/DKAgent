...
3. If `target.recoveryMode` is `ack_intent`, run `lock-status` and recover the matching `owner.token`, copy `ackIntent.expectedDocumentHashes` into the temporary hashes JSON, then retry ACK with exactly `ackRecoveryEventIds`. Do not re-render or adopt documents, change IDs/hashes, release the owner, or acquire another lock. If the exact retry fails, stop with the journal and owner intact.
