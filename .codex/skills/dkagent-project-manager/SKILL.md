...
5. Run `project --token <token> --event-ids <comma-separated-ids>`. For every selected event, copy the returned `block` verbatim into either STATUS or BACKLOG in `target.managementWorktree` and keep it after ACK. The two-line block contains the exact human task facts plus digest-bound canonical event payload; never hand-write, reserialize, abbreviate, or split it across documents. Patch other human-readable sections only as needed.
