...
2. `PRIMARY KEY`、`FOREIGN KEY`、`ON DELETE CASCADE` 分别解决什么问题？
	- `PRIMARY KEY`：保证每条记录有唯一且非空的身份。
	- `FOREIGN KEY`：用来关联两张表，保证 `chunk.parent_id` 对应的父文档必须真实存在。
	- `ON DELETE CASCADE`：父文档删除时，数据库自动删除它的全部子块。
	
3. 为什么更新父文档时，要在事务中删除旧子块并插入新子块？
	事务保证父文档更新、旧子块删除和新子块插入具有原子性；删除全部旧子块再重新插入，则能简单可靠地处理子块的新增、删除、修改和顺序变化，不需要逐个计算差异。
```
...
当前流程也不是逐个删除旧子块，而是一条 SQL 删除全部旧子块：
...
删除全部旧子块再重新插入，是为了简单、可靠地处理子块的新增、修改、删除和顺序变化；不用逐个计算子块差异。
```
...
6. HNSW 索引与余弦相似度分别负责什么？
	HNSW是一种向量算法，近似相邻性算法，快速寻找附近向量，适合 Top-K 检索
	余弦相似度计算两个向量是否相近
...
```text
删除 SSE.md 父记录 → 自动删除 SSE.md 的全部 chunks
```
...
```sql
SELECT source, title, indexed_at -- 选择3列
FROM rag_documents -- 从rag_documents表
ORDER BY indexed_at DESC -- 按索引时间倒序（最新的在前）
LIMIT 5; -- 只返回前5条
```
...
```ts
await pool.query("SELECT source_path, content_hash FROM rag_documents");
```
...
## 15.PostgreSQL 做向量库的优势是什么？

既然 HNSW 别的库也有，那"用 PostgreSQL + pgvector 做 RAG"的价值在哪？主要是这几点：

### 1. 一个数据库搞定"关系 + 向量"两种数据

这是你项目里最核心的优势。你的 `rag_documents` 和 `rag_chunks` 是**关系型的一对多结构**，同时 `embedding` 又是向量。用 PostgreSQL 可以：

```sql
-- 同一条 SQL 里既做向量检索，又做关系过滤
SELECT chunk_id, content
FROM rag_chunks
WHERE parent_id = 'xxx'                    -- 关系型过滤
ORDER BY embedding <=> $query_vector      -- 向量相似度排序
LIMIT 5;
```

专门向量库（Milvus、Pinecone）不擅长这种"向量 + 结构化字段 + JOIN"的混合查询，而 PostgreSQL 天然擅长。

### 2. 复用已有基础设施

- 不用额外部署、运维一个向量数据库服务
- 事务、备份、权限、SQL 都是现成的
- 你的父子文档模型（外键、级联删除）正好用关系模型表达

### 3. 数据一致性更好

向量和它的元数据（父文档、标题、hash）在**同一个事务**里，不会出现"向量写进去了，元数据没写进去"的中间状态。

### 对比

|维度|PostgreSQL + pgvector|专门向量库（Milvus 等）|
|---|---|---|
|混合查询（向量+关系）|✅ 强|❌ 弱|
|运维成本|✅ 低（复用 PG）|❌ 需单独维护|
|十亿级向量规模|⚠️ 一般|✅ 强（分布式）|
|性能调优空间|中等|更强（GPU、分布式）|

你笔记第 14 节也说了当前只有 3335 个子块，规模不大，所以用 PostgreSQL + pgvector 完全够用，还能顺便实践真实向量检索。
