...
```text
“SSE 和普通 HTTP 有什么区别” → SSE.md 排名第一
```

它不能证明：

1. 查询SOP、代码、图片、表格时能否正确命中
2. 是否有其他业务能命中到
3. 如果没有答案是，模型可以采取降级方案么
4. 向量维度1024、Embedding模型选择、子块最大1600字是否合理
5. 目前相似度是否合适
...
{
	"query":"SSE适合哪些场景？",
	"relevantSourcePaths":["C-前端学习/node/SSE.md"],
	"expectedFacts":["适合服务器向客户端单向推送"],
	"shouldRefuse":false
}
```
...
```ts
// 中文注释：只要任意正确父文档进入返回路径，就算该题召回成功
const recalled = relevantSourcePaths.some((sourcePath) =>
...
对于 `shouldRefuse=true` 的问题，当前代码直接设置：

```ts
// 中文注释：拒答题不参与文档召回成功率，所以这里固定记为 true
