...
1. 为什么用户问题也必须调用同一个 Embedding 模型；
	不同模型的语义空间和维度不同，同一个分块对不同向量模型生成的向量也不一致

2. pgvector 如何通过余弦距离排列子块；
	数据库实际按余弦距离从小到大排序：
	
3. 为什么 `Top-3` 不是直接查询 3 个子块，而是先查至少 12 个；
	“先召回至少12个候选子块”，再去重获得3篇父文档。12是工程经验值，需要通过评估校准
	
4. 为什么同一父文档只保留最高分子块；
	1. 增加来源多样性，防止一个父文档占满top-3
	2. 用最高分块代表父文档最相关的位置
	
5. HNSW、余弦距离和父文档聚合分别处于哪一层；
	- HNSW：数据库索引层，快速缩小候选范围，近似检索可能损失少量召回精度。
	- 余弦距离：向量度量层，定义两个向量如何比较。
	- 父文档聚合：应用逻辑层，在Node.js中完成去重和Top-K选择。

测验

问题1: 数据库返回如下，父文档聚合并取 Top-3 后，保留哪些子块？
```text
A/a1 0.91
A/a2 0.88
B/b1 0.86
...
问题2:某子块的余弦距离是 `0.24`，CLI 显示的相似度是多少？它是否代表答案有 76% 的概率正确？
回答：
相似度：0.76，不代表，只代表选出来的子块和问题有76%的相似

问题3:HNSW 和余弦距离分别解决什么问题？
回答：
HNSW是解决如何快速找到子块，
余弦距离解决找哪些子块
...
回答：
检索命中也不等于回答正确。还需要检查证据是否完整、是否真正支持结论、是否依赖未解析图片，以及LLM是否产生幻觉，并通过引用校验和拒答策略控制最终输出。
...
> 
> 把用户问题转成的向量 `$1` 送进来，在 `rag_chunks` 里找出**语义最接近的前 `$2` 个子块**，同时把每个子块对应的**父文档信息**一起带出来。
> 
> 检索对象：子文档块
> 
> 优点：检索更加准确，主题更加集中
...
> 检索策略：余弦相似度
> 
> 在pgvector中，<=>是计算余弦距离，1-距离是余弦相似度
> `1-距离`的原因：距离的区间是[0,2]，用`1-距离`，可以将结果映射到[-1,0,1]
> 
> 父文档：通过parent.id获取父文档标题
> 
...
| 行                                                | 含义                                              |
| ------------------------------------------------ | ----------------------------------------------- |
| `c.parent_id`                                    | 子块所属的父文档 ID                                     |
| `c.source_path`                                  | 文档路径，你的子块表里冗余存了这个字段，方便直接查                       |
| `d.title AS document_title`                      | 父文档标题，来自 `rag_documents` 表，取别名 `document_title` |
| `c.id AS chunk_id`                               | 子块自己的 ID，取别名 `chunk_id`                         |
| `c.heading_path`                                 | 标题路径（如 `['前端','网络','SSE']`），记录子块在文档中的层级位置       |
| `c.content`                                      | 子块的正文内容——**这是最终要喂给 LLM 的文本**                    |
| `1 - (c.embedding <=> $1::vector) AS similarity` | 计算相似度（下面重点讲）                                    |
| `c.needs_vision`                                 | 该子块是否包含图片、需要视觉模型处理                              |
...
1. **`<=>`**：pgvector 的**余弦距离**运算符。计算两个向量之间的"距离"
2. **`$1`**：问题向量
3. `::vector` 把它显式转成 `vector` 类型
4. **`1 - 距离`**：把距离换算成相似度
...
所以 `1 - 距离` 让你得到"越大越相似"的直观数值，距离 0 → 相似度 1。
...
> 简述：
> 每个父文档保留相似度最高的子块，按照子块相似度进行降序排序，选出top-k个父文档
