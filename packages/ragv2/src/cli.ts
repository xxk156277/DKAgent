...
function readQuery(args: string[]): string {
  return args.filter((value, index) => !value.startsWith("--") && !args[index - 1]?.startsWith("--")).join(" ").trim();
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (command === "search") {
      ...
      console.log(`Top-${topK}，耗时 ${result.durationMs}ms，Embedding tokens=${result.embeddingTokens ?? "未知"}`);
    ...
    if (command === "ask") {
      ...
      console.log("检索来源：");
      result.hits.forEach((hit, index) =>
        console.log(`[${index + 1}] ${hit.sourcePath}#${hit.headingPath.join(" > ") || "全文"} (${hit.similarity.toFixed(4)})`),
      );
      return;
    }
