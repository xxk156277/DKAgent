...
describe("TapApp", () => {
    ...
    it("keeps the collapsed Agent insights toggle within its 48px rail", () => {
        ...
        expect(styles).toMatch(/\.tap-insights-rail\.is-collapsed\s+\.ant-btn\s*\{[^}]*min-inline-size:\s*32px;/s);
        expect(styles).toMatch(
...
function agentMetricsFixture(): TraceEvent[] {
    return [
        ...
        metricEvent(
            ...
            "start",
            { input: { step: 1 } },
            { spanId: "step-1", parentSpanId: "turn-1", step: 1 },
        ),
        metricEvent(
            "metric-3",
            "turn-1",
        ...
        metricEvent(
            ...
            "end",
            { output: {} },
            { spanId: "step-1", parentSpanId: "turn-1", step: 1, durationMs: 40 },
        ),
        metricEvent(
            "metric-9",
            "turn-1",
...
function agentMetricsFixtureWithRunningTurn(): TraceEvent[] {
    return [
        ...
        metricEvent(
            ...
            "start",
            { input: { step: 1 } },
            { spanId: "step-2", parentSpanId: "turn-2", step: 1 },
        ),
    ];
}
...
function fixtureEvents(): TraceEvent[] {
    let sequence = 0;
    const event = (id: string, type: string, turnId: string, payload: unknown, step?: number): TraceEvent => {
