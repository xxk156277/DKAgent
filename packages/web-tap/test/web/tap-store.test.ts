...
import {
    ...
    selectTurns,
    useTapStore,
} from "../../src/web/store/tap-store.js";
...
describe("createTapStore", () => {
    it("selects the latest Turn and latest node on first history load", () => {
        ...
        store.getState().replaceHistory([event("first", "turn-1", 1), event("latest", "turn-2", 2)]);
    ...
    it("pauses on a historical node and resumes from the latest node", () => {
        const store = createTapStore();
        store.getState().replaceHistory([event("started", "turn-1", 1), event("finished", "turn-1", 2, "turn.end")]);
    ...
    it("merges successful history responses from the current connection", async () => {
        ...
        const fetch = vi.fn().mockReturnValueOnce(firstHistory.promise).mockReturnValueOnce(secondHistory.promise);
    ...
    it("keeps completed history when a later current-connection request fails", async () => {
        ...
        const fetch = vi.fn().mockReturnValueOnce(initialHistory.promise).mockReturnValueOnce(refreshedHistory.promise);
