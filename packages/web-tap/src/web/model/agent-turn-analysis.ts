...
export function analyzeAgentTurn(turn: TapTurnView): AgentTurnAnalysis {
    ...
    const compactions = events.filter(
        (event) => event.name === "context.compaction.completed" && event.phase === "event",
    );
...
function readToolSuccess(calls: TraceEvent[], results: TraceEvent[]): { successCount: number } | undefined {
...
function evaluateTurnStatus(status: AgentTurnMetrics["status"], terminal: TraceEvent | undefined): AgentEvaluationItem {
    if (status === "completed") {
    ...
    if (status === "error") {
        return item(
            ...
            "Turn 完成状态",
            "failed",
            readErrorMessage(terminal) ?? "本轮执行失败",
...
function evaluateModelPairs(
    ...
    const requestSpanIds = new Set(
        requests.map((event) => event.spanId).filter((id): id is string => id !== undefined),
    );
    const responseSpanIds = new Set(
        directResponses.map((event) => event.spanId).filter((id): id is string => id !== undefined),
    );
...
function evaluateToolResults(
    ...
    if (success.successCount !== calls.length) {
        return item(
            ...
            "Tool 执行结果",
            "failed",
            `${calls.length - success.successCount} 个 Tool 调用明确失败`,
            evidence,
        );
    }
...
function evaluateLoop(
    ...
    if (status === "completed") {
        return item(
            ...
            "循环效率",
            "passed",
            `本轮在 ${stepCount} 个 Step 内完成；不判断该路径是否最优`,
            [],
        );
    }
...
function evaluateCompaction(latest: AgentTurnMetrics["latestCompaction"], events: TraceEvent[]): AgentEvaluationItem {
    ...
    if (latest.tokensAfter < latest.tokensBefore) {
        return item(
            ...
            "Context 压缩结果",
            "passed",
            `Token 从 ${latest.tokensBefore} 降至 ${latest.tokensAfter}；不代表语义一定完整`,
            evidence,
        );
    }
    return item(
