...
import type { TapModuleKind, TapNodeKind, TapNodeView, TapSessionView, TapStepView, TapTurnView } from "./types.js";
...
function toNodes(event: TraceEvent, step: number, before?: TraceEvent): TapNodeView[] {
    ...
    if (event.name === "context.snapshot.created") {
        ...
        return dropped > 0 && before ? [derivedTrimNode(before, event, step), after] : [after];
    }

    const definition = nodeDefinition(event);
    return [eventNode(event, step, definition.kind, definition.title, definition.status, unwrapData(event.data))];
}
...
function nodeDefinition(event: TraceEvent): {
    ...
    switch (event.name) {
        ...
        case "agent.step":
            return { kind: "step_start", title: `Step ${event.step ?? 1} 开始`, status: "running" };
        case "context.build":
            return { kind: "context_before", title: "构建 Context", status: "running" };
        case "context.tokens.counted":
            return { kind: "context_tokens", title: tokenStageLabel(event.data), status: "completed" };
        case "context.threshold.checked":
            return { kind: "context_threshold", title: "检查压缩阈值", status: "completed" };
        case "context.compaction.planned":
            return { kind: "context_compaction_plan", title: "规划 Context 压缩", status: "completed" };
        case "context.summary.request":
            return { kind: "context_summary_request", title: "摘要模型请求", status: "running" };
        case "context.summary.response":
            return { kind: "context_summary_response", title: "摘要模型响应", status: "completed" };
        case "context.compaction.completed":
            return { kind: "context_compaction_completed", title: "Context 压缩完成", status: "completed" };
        case "model.request":
            return { kind: "model_request", title: modelTitle(event, "请求"), status: "running" };
        case "model.response":
            return { kind: "model_response", title: modelTitle(event, "响应"), status: "completed" };
        case "tool.call":
            return { kind: "tool_call", title: "Tool 调用", status: "running" };
        case "tool.result":
            return { kind: "tool_result", title: "Tool 结果", status: "completed" };
        default:
            return { kind: "unknown", title: "未知事件", status: "completed" };
    }
}
...
function modelTitle(event: TraceEvent, phase: "请求" | "响应"): string {
    const operation =
        event.module === "memory" || event.module === "skill" ? operationLabel(event.operation) : undefined;
...
function derivedTrimNode(before: TraceEvent, after: TraceEvent, step: number): TapNodeView {
    ...
    const afterContext = isRecord(afterData) && "context" in afterData ? afterData.context : afterData;
    return {
