...
import { selectNodes, selectTurns, tapStore, type TapState } from "../store/tap-store.js";
...
export function TapApp({ store = tapStore, sessionId, connectLive = true, sessionNavigation }: TapAppProps) {
    ...
    useEffect(
        () => (connectLive ? connectEventFeed(store, sessionId ? { sessionId } : {}) : undefined),
    ...
    const attentionCount =
        turnAnalysis?.evaluations.filter((item) => item.status === "warning" || item.status === "failed").length ?? 0;
            ...
            <AntdApp className="tap-ant-app">
                <div className="tap-app-shell">
                    ...
                    <Drawer
                        ...
                        destroyOnHidden
                        extra={
                            <Button type="text" aria-label="关闭对话轮次" onClick={() => setTurnsOpen(false)}>
                                关闭
                            </Button>
                        }
