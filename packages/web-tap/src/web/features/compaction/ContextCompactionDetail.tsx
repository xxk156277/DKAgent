...
export function ContextCompactionDetail({ diff, rawEvents }: ContextCompactionDetailProps) {
  ...
  const items = [
    ...
    { key: "max", label: "最大上下文 Token", children: formatNumber(diff.beforeMaxContextTokens ?? diff.afterMaxContextTokens) },
    { key: "reserved", label: "预留输出 Token", children: formatNumber(diff.beforeReservedOutputTokens ?? diff.afterReservedOutputTokens) },
    { key: "estimated", label: "估算输入 Token", children: formatTransition(diff.beforeEstimatedInputTokens, diff.afterEstimatedInputTokens) },
    { key: "available", label: "可用输入 Token", children: formatTransition(diff.beforeAvailableInputTokens, diff.afterAvailableInputTokens) },
  ];
