...
export async function startTapServer(options: {
    ...
    try {
        ...
        server = createServer((request, response) => {
            void handleRequest(request.url, response, options.store, options.sessions, clients, webRoot);
        });
    ...
    try {
        ...
        if (sessionEventsMatch) {
            ...
            sendJson(
                response,
                200,
                store.list().filter((event) => event.sessionId === sessionId),
            );
            return;
        }
        ...
        const file =
            (await resolveStaticFile(pathname, webRoot)) ??
