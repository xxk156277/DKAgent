...
import { OpenAICompatibleEmbeddingProvider, createEmbeddingProviderFromEnv } from "../../src/knowledge/embedding.js";
...
test("Provider 校验响应数量、维度和有限数字", async () => {
    ...
    await assert.rejects(createProvider([{ index: 0, embedding: [1, 0] }]).embedBatch(["a", "b"]), /数量不一致/);
    ...
    await assert.rejects(createProvider([{ index: 0, embedding: [Number.NaN] }]).embedBatch(["a"]), /有限数字/);
});
