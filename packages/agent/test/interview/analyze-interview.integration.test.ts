...
test("真实 Registry 分析 500 行以上文字稿并写出完整暂定报告", async () => {
    ...
    try {
        ...
        const requestInputs = provider.requests.map((request) => {
            const message = request.messages[0];
            return message?.role === "user" ? JSON.parse(message.content) : null;
        });
        assert.equal(Array.isArray(requestInputs[0]), true);
        assert.equal(requestInputs[0][0].content, "请介绍你的低代码项目");
        assert.equal("correctedContent" in requestInputs[0][0], false);
        const questionRequests = requestInputs.filter((input) => input?.expressionStats);
        assert.equal(questionRequests.length, 2);
        assert.equal(questionRequests[0].expressionStats.characterCount > 500, true);
        assert.equal(questionRequests[1].expressionStats.characterCount, 10);
