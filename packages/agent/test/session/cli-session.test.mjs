...
const toolRegistryModuleUrl = pathToFileURL(join(repositoryRoot, "packages/agent/src/tools/registry.ts")).href;

function startCli(workingDirectory, { runAgentOptions = "", scriptPreamble = "", environment = {} } = {}) {
    const script = `
    ...
    const waitForStreamOutput = (getOutput, stream, pattern, fromIndex = 0) =>
    ...
    const waitForOutput = (pattern, fromIndex = 0) =>
        waitForStreamOutput(() => stdout, child.stdout, pattern, fromIndex);
    const waitForErrorOutput = (pattern, fromIndex = 0) =>
        waitForStreamOutput(() => stderr, child.stderr, pattern, fromIndex);
...
test("CLI 启动时创建 Session，输入 /new 后切换到新 Session", () => {
    ...
    const sessionIds = [...result.stdout.matchAll(/Session ([0-9a-f-]{36})/g)].map((match) => match[1]);
    assert.equal(sessionIds.length, 2, result.stdout);
    assert.notEqual(sessionIds[0], sessionIds[1]);
    assert.equal(existsSync(join(workingDirectory, ".dkagent/sessions.db")), true);
});
...
test("CLI 为 /new 创建独立 ArtifactStore，并在 /switch 时由 Tool 观察到原 Store", async () => {
    ...
    try {
        ...
        await cli.waitForOutput(new RegExp(`已切换到 Session ${firstSessionId}`), switchOutputIndex);
...
function createArtifactCaptureServer() {
    return createServer(async (request, response) => {
        ...
        const hasCaptureTool = parsed.tools?.some((tool) => tool.function?.name === "capture_store");
        const latestMessage = parsed.messages?.at(-1);
        const event =
            hasCaptureTool && latestMessage?.role !== "tool"
                ? {
                      choices: [
                          {
                              delta: {
                ...
                : {
                      choices: [
                          {
                              delta: {
                                  content: hasCaptureTool ? "captured" : JSON.stringify({ memories: [] }),
                              },
                              finish_reason: "stop",
                          },
                      ],
...
test("CLI 可以列出、切换和删除非当前 Session", async () => {
    ...
    await cli.waitForOutput(new RegExp(`\\* ${secondSessionId}[\\s\\S]*  ${firstSessionId}`), outputIndex);
    ...
    outputIndex = cli.output().length;
    cli.child.stdin.write(`/switch ${secondSessionId}\n`);
    await cli.waitForOutput(new RegExp(`已切换到 Session ${secondSessionId}`), outputIndex);

    outputIndex = cli.output().length;
    cli.child.stdin.write(`/delete ${firstSessionId}\n`);
    await cli.waitForOutput(new RegExp(`已删除 Session ${firstSessionId}`), outputIndex);
...
test("CLI 切换结果决定后续用户消息写入的 Session，并支持 Tab 分隔参数", async () => {
    ...
    try {
        ...
        await cli.waitForOutput(new RegExp(`已切换到 Session ${targetSessionId}`), outputIndex);
        ...
        await cli.waitForOutput(new RegExp(`已删除 Session ${disposableSessionId}`), outputIndex);
    } finally {
        const exitPromise = cli.waitForExit();
        cli.child.stdin.end();
        const exitCode = await exitPromise;
