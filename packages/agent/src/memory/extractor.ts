...
export class MemoryExtractor {
    ...
    public constructor(
        ...
        const inherited = queryEngine.getTracer?.();
        if (tracer !== undefined && inherited !== undefined && tracer !== inherited) {
            throw new Error("MemoryExtractor tracer must match QueryEngine tracer");
        }
    ...
    public async extract(input: MemoryCaptureInput): Promise<MemoryCandidate[]> {
        ...
        return this.tracer.span(
            "memory.extract",
            { userInput: input.userInput, assistantAnswer: input.assistantAnswer },
            async (extractSpan) => {
                let response: ModelResponse;
                try {
                    response = await this.queryEngine.query(request);
                } catch {
                ...
                const parsed = response.type === "tool_use"
                    ...
                    : { candidates: [] };
                extractSpan.setOutput({ candidates: parsed.candidates as unknown as JsonValue[] });
