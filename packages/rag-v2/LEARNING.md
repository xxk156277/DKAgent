...
对每个失败只归入一个首要原因：数据缺失、标题切块、查询与术语不一致、父文档聚合、图片缺失。当前已增加 BM25 + Dense + RRF，应使用 `--strategy dense|hybrid` 同集对照；仍不添加重排。
...
检查回答中的每个 `[n]` 是否真正支持前面的结论。短父文档返回全文；长父文档先保留命中块，再用剩余预算加入相邻块并去除 overlap；总上下文受 6000 字符预算约束。系统会增加一次模型语义检查，但仍需人工抽查模型裁判。
