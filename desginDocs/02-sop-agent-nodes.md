...
| 内部原子 Tool | 完成预处理、项目事实、表达、逐题分析和报告生成 | 不决定后续调用顺序，不注册到公共 ToolRegistry |
| LLM | 在现有 Schema、Rubric 和证据校验内完成纠错、结构化及语义分析 | 不修改原稿，不决定工作流，不直接写文件、Session 或 Memory |

公共 ToolRegistry 当前只注册：`read_file`、`find_files`、`grep_files`、`write_file`、`analyze_interview`。`preprocess_transcript`、`extract_project_facts`、`analyze_expression`、`analyze_answer`、`generate_report` 只由 Skill 持有。
...
3. Skill 使用分页 `read_file` 读完文字稿（默认每页 500 行），解析角色和原文位置，执行高置信纠错与问题/问题簇结构化。
4. Skill 仅对项目问题簇提取项目事实；对非流程题分析表达，知识题可使用可选 FTS 参考资料，再按原文顺序逐题分析。单题、项目事实和知识检索失败均按既有降级策略继续。
