...
test("支持 FTS、Embedding 和 Hybrid RRF 三种检索", async () => {
  ...
  try {
    ...
    assert.deepEqual(fts.map((item) => item.entry.id), ["js#1"]);
    assert.equal(fts[0]?.matchType, "fts");

    const semantic = await search.search({ query: "异步机制", method: "embedding" });
    assert.deepEqual(semantic.map((item) => item.entry.id), ["js#2", "css#1", "js#1"]);
...
test("Embedding 检索支持维度过滤并拒绝向量维度不一致", async () => {
  ...
  try {
    ...
    assert.deepEqual(results.map((item) => item.entry.id), ["js#1"]);
    ...
    await assert.rejects(
      invalid.search({ query: "作用域", method: "embedding" }),
      /向量维度不一致/,
    );
  } finally {
    database.close();
...
test("Embedding 模式未配置 Provider 时明确失败", async () => {
  ...
  try {
    ...
    await assert.rejects(
      search.search({ query: "闭包", method: "embedding" }),
      /Embedding Provider/,
    );
  } finally {
    database.close();
